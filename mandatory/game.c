...
void     draw_player(t_cub *cub)
{
    int i = -4;
    int j = -4;
    int screen_x = cub->ply.ply_x * TILE;
    int screen_y = cub->ply.ply_y * TILE;
    ...
    while(j < 4)
    {
        ...
        while(i < 4)
        {
            if(inside_bounds(*cub, (screen_x + j)/TILE, (screen_y + i)/TILE) && screen_bounds(screen_x + j, screen_y + i) && (i * i + j * j <= 16))
