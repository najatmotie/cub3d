...
void key_hook(void *param)
{
    ...
    {
        // printf("jjjjj\n");
        new_x = cub->ply.pos_x + cos(degree_to_radian(cub->ply.angle)) * MOVE_SPEED;
        new_y = cub->ply.pos_y - sin(degree_to_radian(cub->ply.angle)) * MOVE_SPEED;
	...
	{
        new_x = cub->ply.pos_x + cos(degree_to_radian(cub->ply.angle) + M_PI / 2) * MOVE_SPEED;
        new_y = cub->ply.pos_y - sin(degree_to_radian(cub->ply.angle) + M_PI / 2) * MOVE_SPEED;
        if(inside_bounds(cub, new_x, new_y) && !cub->map.map[(int)new_y][(int)new_x])
        {
            // printf("A %f\n", degree_to_radian(cub->ply.angle) + M_PI / 2);
            cub->ply.pos_x = new_x;
            cub->ply.pos_y = new_y;
        }
    }	
